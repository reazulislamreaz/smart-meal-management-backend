...
describe('MealPlansService', () => {
  ...
  let createdPlanItems: Array<{ mealId: string; dayOfWeek: number; mealType: string }>;
  ...
  const buildAiMeals = (
    frequency: { breakfast: number; lunch: number; dinner: number },
  ) => {
    const meals: any[] = [];
    const pushMeals = (mealType: string, count: number) => {
      for (let index = 0; index < count; index++) {
        meals.push({
          ...
          instructions: ['Cook'],
          ingredients: [{ name: 'Rice', quantity: '1 cup', category: 'Pantry' }],
        });
      }
    };
